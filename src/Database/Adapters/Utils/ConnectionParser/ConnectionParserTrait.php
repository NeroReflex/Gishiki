...
trait ConnectionParserTrait
{
    ...
    public function getPDOConnection()
    {
        ...
        $query .= ((is_string($this->host)) && (strlen($this->host) > 0)) ? 'host=' . $this->host . ';' : '';
        $query .= ((is_string($this->port)) && (strlen($this->port) > 0)) ? 'port=' . $this->port . ';' : '';

        $dbName = ((is_string($this->user)) && (strlen($this->user) > 0)) ? $this->user : null;
