...
abstract class Cryptography
{
    ...
    public static function encrypt(SecretKey &$key, $message, $initVector = null, $algorithm = self::AES_CBC_128)
    {
        ...
        if (!self::checkKeySize($managedKey, $algorithm)) {
            if ($algorithm == self::AES_CBC_128) {
                throw new SymmetricException('You must be using a key with the correct length for the chosen algorithm', 3);
            } elseif ($algorithm == self::AES_CBC_192) {
                throw new SymmetricException('You must be using a key with the correct length for the chosen algorithm', 4);
            } elseif ($algorithm == self::AES_CBC_256) {
                throw new SymmetricException('You must be using a key with the correct length for the chosen algorithm', 5);
            }
        }
